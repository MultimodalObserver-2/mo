...
def test_open_file_windows(file_mgmt):
    with (
        patch("mo.core.file_management.file_management.platform.system", return_value="Windows"),
...
def test_delete_directory_with_readonly(mock_exists, mock_rmtree, file_mgmt):
    ...
    mock_rmtree.side_effect = lambda path, onexc: onexc(mock_onexc_func, "/internal/path", None)
...
def test_copy_file_obj(mock_copyfileobj, file_mgmt):
    ...
    with patch("builtins.open") as mock_file:
        result = file_mgmt.copy_file_obj(mock_file_obj, name, rel_path)
...
def test_normalize_file_name():
    assert FileManagement.normalize_file_name("Tést Fíle Nãme") == "TestFileName"
    assert (
        FileManagement.normalize_file_name("file_with_special-chars!@#") == "file_with_specialchars"
    )
    assert FileManagement.normalize_file_name("another_file_123") == "another_file_123"
