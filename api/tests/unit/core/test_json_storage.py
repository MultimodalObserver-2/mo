...
from api.core.file_management.exceptions import InvalidFileNameError, NotFoundError


@pytest.fixture
def json_storage():
    with patch("api.core.file_management.json_storage.JsonStorage.create_storage", return_value="test.json"), \
...
def test_create_storage_success():
    with patch("api.core.file_management.json_storage.FileValidators.is_valid_file_name", return_value=True), \
...
def test_create_storage_invalid_name():
    with patch("api.core.file_management.json_storage.FileValidators.is_valid_file_name", return_value=False):
...
def test_insert_one(json_storage):
    ...
    with patch("api.core.file_management.json_storage.open", mock):
        with patch("api.core.file_management.json_storage.json.load", return_value=data_before), \
...
def test_find_all(json_storage):
    expected = [{"id": 1}]
    with patch("api.core.file_management.json_storage.open", mock_open(read_data=json.dumps(expected))), \
...
def test_update_success(json_storage):
    ...
    with patch("api.core.file_management.json_storage.JsonStorage.find_all", return_value=initial_data), \
...
def test_update_not_found(json_storage):
    with patch("api.core.file_management.json_storage.JsonStorage.find_all", return_value=[]), \
