...
def test_add_plugin_success(plugin_service):
    ...
    plugin_service.plugin_manager.register_plugin.assert_called_with("my_plugin_abc")
    plugin_service.plugins_dir_handler.add_known_dir.assert_called_with("my_plugin_abc")
...
def test_add_plugin_register_fails(plugin_service):
    ...
    plugin_service.file_management.get_unique_name.return_value = "my_plugin_abc"
    plugin_service.plugin_manager.register_plugin.side_effect = Exception("Registration failed")

    with pytest.raises(BadRequestException, match="Failed to add plugin"):
        plugin_service.add_plugin(mock_file)

    plugin_service.file_management.delete_directory.assert_called_with("my_plugin_abc")


def test_get_all_plugins(plugin_service):
    mock_plugins_list = [MagicMock(spec=PluginMetadata), MagicMock(spec=PluginMetadata)]
...
def test_get_plugin_success(plugin_service):
    ...
    plugin_service.plugin_manager.get_plugin_metadata.assert_called_with("some.id")
...
def test_remove_plugin_success(plugin_service):
    ...
    plugin_service.plugins_dir_handler.suspend.assert_called_once()
    plugin_service.plugin_manager.remove_plugin.assert_called_with("plugin.to.remove")
    plugin_service.file_management.delete_directory.assert_called_with("dir_to_delete")
    plugin_service.plugins_dir_handler.remove_known_dir.assert_called_with("dir_to_delete")
    plugin_service.plugins_dir_handler.resume.assert_called_once()
...
def test_remove_plugin_handles_cleanup_exception(plugin_service):
    plugin_service.plugin_manager.get_plugin_metadata.return_value = MagicMock()
    plugin_service.plugin_manager.get_plugin_dir_name.return_value = "dir_to_delete"
    plugin_service.plugin_manager.remove_plugin.side_effect = Exception("Cleanup failed")

    with pytest.raises(BadRequestException, match="Failed to remove plugin"):
        plugin_service.remove_plugin("plugin.to.remove")

    plugin_service.plugin_manager.register_plugin.assert_called_with("dir_to_delete")
    plugin_service.plugins_dir_handler.add_known_dir.assert_called_with("dir_to_delete")
...
def test_get_plugin_properties_not_found(plugin_service):
    ...
    plugin_service.plugin_manager.get_plugin_metadata.assert_called_with("non.existent.id")
