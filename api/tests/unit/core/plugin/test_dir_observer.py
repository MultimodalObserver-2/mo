...
def plugins_dir_handler():
    ...
    handler.plugin_management = mock_manager_instance
    yield handler
...
def test_on_created_success(mock_relpath, plugins_dir_handler):
    ...
    plugins_dir_handler.plugin_management.register_plugin.assert_called_with("new_plugin")
...
def test_on_created_suspended(mock_relpath, plugins_dir_handler):
    ...
    plugins_dir_handler.plugin_management.register_plugin.assert_not_called()
...
def test_on_deleted_success(mock_relpath, plugins_dir_handler):
    ...
    plugins_dir_handler.plugin_management.remove_plugin_by_dir.assert_called_with("deleted_plugin")
...
def test_on_deleted_unknown_dir(mock_relpath, plugins_dir_handler):
    ...
    plugins_dir_handler.plugin_management.remove_plugin_by_dir.assert_not_called()
...
def test_on_moved_success(mock_relpath, plugins_dir_handler):
    ...
    plugins_dir_handler.plugin_management.rename_plugin_dir.assert_called_with("new_name")
...
def test_on_created_wait_for_file_fails(mock_print, mock_relpath, plugins_dir_handler):
    event = DirCreatedEvent("/fake/path/new_plugin")
    plugins_dir_handler.wait_for_file = MagicMock(return_value=False)

    plugins_dir_handler.on_created(event)

    mock_print.assert_called_with("ERROR: Metadata file not found for plugin new_plugin")
    plugins_dir_handler.plugin_management.register_plugin.assert_not_called()


@patch("os.path.relpath", return_value="new_plugin")
@patch("builtins.print")
def test_on_created_handles_exception(mock_print, mock_relpath, plugins_dir_handler):
    event = DirCreatedEvent("/fake/path/new_plugin")
    plugins_dir_handler.wait_for_file = MagicMock(return_value=True)
    plugins_dir_handler.plugin_management.register_plugin.side_effect = Exception(
        "Registration Failed"
    )
    ...
    assert "ERROR: Failed to load plugin new_plugin" in mock_print.call_args_list[0][0][0]
...
@patch("os.path.relpath", return_value="deleted_plugin")
@patch("builtins.print")
def test_on_deleted_handles_exception(mock_print, mock_relpath, plugins_dir_handler):
    plugins_dir_handler.known_dirs.append("deleted_plugin")
    event = DirDeletedEvent("/fake/path/deleted_plugin")
    plugins_dir_handler.plugin_management.remove_plugin_by_dir.side_effect = Exception(
        "Removal Failed"
    )
    ...
    mock_print.assert_called()
    assert "ERROR: Failed to remove plugin deleted_plugin" in mock_print.call_args_list[0][0][0]
...
def test_on_moved_suspended(mock_relpath, plugins_dir_handler):
    ...
    plugins_dir_handler.plugin_management.rename_plugin_dir.assert_not_called()


@patch("os.path.relpath", side_effect=["old_name", "new_name"])
@patch("builtins.print")
def test_on_moved_handles_exception(mock_print, mock_relpath, plugins_dir_handler):
    plugins_dir_handler.known_dirs = ["old_name"]
    event = DirMovedEvent("/fake/path/old_name", "/fake/path/new_name")
    plugins_dir_handler.plugin_management.rename_plugin_dir.side_effect = Exception("Rename Failed")
    ...
    mock_print.assert_called()
    assert (
        "ERROR: Failed to rename plugin old_name to new_name" in mock_print.call_args_list[0][0][0]
    )
