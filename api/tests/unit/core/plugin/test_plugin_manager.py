...
def test_register_plugin_success(mock_load_meta, mock_process, mock_queue, plugin_manager):
    ...
    mock_status_queue.get.return_value = {"is_loaded": True, "plugin_types": [Plugin]}
...
def test_get_plugins_metadata_from_type_success(plugin_manager):
    ...
    plugin_manager.plugins_metadata = {"plugin_a": mock_meta_a, "plugin_b": mock_meta_b}
...
def test_get_entry_point(plugin_manager):
    metadata_dict = {"entryPoints": {"mo.plugin": "my_module:MyClass"}}
    with patch.object(plugin_manager, "load_metadata_file", return_value=metadata_dict):
        ...
        none_result = plugin_manager.get_entry_point("plugin_a", "non.existent")
...
def test_exists_entry_point(plugin_manager):
    ...
    with patch.object(plugin_manager, "load_metadata_file", return_value=metadata_dict):
        assert plugin_manager.exists_entry_point("plugin_a", "mo.plugin") is True
        assert plugin_manager.exists_entry_point("plugin_a", "non.existent") is False
...
def test_remove_plugin_success(plugin_manager):
    with patch.object(
        plugin_manager, "_get_plugin_metadata_dir", return_value="plugin_a"
    ) as mock_get_dir:
...
def test_get_active_plugin_process_metadata_is_none(plugin_manager):
    ...
    plugin_manager._get_plugin_process_metadata_dir = MagicMock(return_value="plugin_a")
    result = plugin_manager.get_active_plugin_process("id")
    assert result is None
...
def test_get_plugin_properties_success(plugin_worker_process, plugin_manager):
    ...
    plugin_manager.plugin_processes = {"plugin_a": mock_process}
    plugin_manager._get_plugin_metadata_dir = MagicMock(return_value="plugin_a")
    plugin_manager.plugin_processes_metadata = MagicMock()
...
def test_validate_plugin_settings_process_is_none(mock_worker_class, plugin_manager):
    ...
    mock_worker_class.return_value.validate_settings.assert_called_with(mock_settings)
...
def test_remove_plugin_dir_name_is_none(plugin_manager):
    ...
    plugin_manager._get_plugin_metadata_dir.assert_called_with("non_existent.id")


def test_get_plugin_process_key_is_none(plugin_manager):
    plugin_manager._get_plugin_process_metadata_dir = MagicMock(return_value=None)

    result = plugin_manager.get_plugin_process("non_existent.id")

    assert result is None
    plugin_manager._get_plugin_process_metadata_dir.assert_called_with("non_existent.id")
