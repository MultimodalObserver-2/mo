...
def test_add_capture_config_success(config_service, mock_plugin_metadata):
    ...
    with patch("mo.modules.capture.services.config_service.JsonStorage") as mock_json_storage_class:
        ...
        result = config_service.add_capture_config(project_name, config_req)
        ...
        config_service.project_service.exists.assert_called_with(project_name)
...
def test_add_capture_config_project_not_found(config_service):
    ...
    settings_data = {"name": "TestSetting", "plugin_id": "pub.plugin", "settings": {}}
...
def test_add_capture_config_plugin_not_found(config_service):
    project_name = "TestProject"
    settings_data = {"name": "TestSetting", "plugin_id": "pub.non_existent_plugin", "settings": {}}
    config_req = CaptureConfigPostReq(**settings_data)
...
def test_add_capture_config_already_exists(config_service):
    project_name = "TestProject"
    settings_data = {"name": "ExistingSetting", "plugin_id": "pub.plugin", "settings": {}}
    config_req = CaptureConfigPostReq(**settings_data)
...
def test_add_capture_config_plugin_metadata_not_found(config_service):
    project_name = "TestProject"
    config_req = CaptureConfigPostReq(name="TestSetting", plugin_id="pub.plugin", settings={})
...
def test_get_capture_config_success(config_service, mock_plugin_metadata):
    ...
    config_dict = {"name": config_name, "plugin_id": "pub.plugin1", "settings": {"key": "val"}}
    ...
    with patch("mo.modules.capture.services.config_service.JsonStorage") as mock_json_storage_class:
        ...
        result = config_service.get_capture_config(project_name, config_name)

        assert result.name == config_name
        assert result.plugin_id == "pub.plugin1"
        mock_storage_instance.find_one.assert_called_with({"name": config_name})
...
def test_get_capture_config_not_found(config_service):
    ...
    with patch("mo.modules.capture.services.config_service.JsonStorage") as mock_json_storage_class:
        ...
        with pytest.raises(NotFoundException):
            config_service.get_capture_config(project_name, config_name)
...
def test_update_capture_config_success(config_service):
    project_name = "TestProject"
    config_name = "MySetting"
    update_data = {"name": "UpdatedSetting", "settings": {"key": "new_val"}}
    ...
    with patch("mo.modules.capture.services.config_service.JsonStorage") as mock_json_storage_class:
        ...
        result = config_service.update_capture_config(project_name, config_name, update_req)

        assert result.name == "UpdatedSetting"
        assert result.settings == {"key": "new_val"}
        config_service.get_capture_config.assert_called_with(project_name, config_name)
        mock_storage_instance.update.assert_called_with({"name": config_name}, ANY)
...
def test_update_capture_config_invalid_settings_validation(config_service):
    ...
    with pytest.raises(BadRequestException):
        config_service.update_capture_config(project_name, config_name, update_req)
...
def test_delete_capture_config_success(config_service):
    ...
    with patch("mo.modules.capture.services.config_service.JsonStorage") as mock_json_storage_class:
        ...
        config_service.delete_capture_config(project_name, config_name)

        config_service.exists.assert_called_with(project_name, config_name)
        mock_storage_instance.delete_one.assert_called_with({"name": config_name})
...
def test_get_all_configs_loaded_success(config_service, mock_plugin_metadata):
    ...
    settings_list = [
        {"name": "Loaded", "plugin_id": mock_plugin_metadata.get_final_id(), "settings": {}},
        {"name": "Unloaded", "plugin_id": unloaded_metadata.get_final_id(), "settings": {}},
    ]
    ...
    with patch("mo.modules.capture.services.config_service.JsonStorage") as mock_json_storage_class:
        ...
        result = config_service.get_all_configs_loaded(project_name)
