...
@plugin_router.post("/", response_model=PluginRes, status_code=status.HTTP_201_CREATED, response_model_exclude_none=True)
async def add_plugin(file: UploadFile, service: PluginService = Depends()):
    return service.add_plugin(file)


@plugin_router.get("/", response_model=list[PluginRes], status_code=status.HTTP_200_OK, response_model_exclude_none=True)
async def get_all_plugins(service: PluginService = Depends()):
    return service.get_all_plugins()


@plugin_router.get(
    "/{plugin_name}/{plugin_version}", response_model=PluginRes, status_code=status.HTTP_200_OK, response_model_exclude_none=True
)
