...
from api.modules.organization.schemas.participant import ParticipantPostReq, ParticipantRes
from api.modules.organization.services.paths import PARTICIPANTS_DATA_FILE_NAME, PROJECTS_DATA_FILE_NAME, PROJECTS_DIR_NAME
...
class ParticipantService:
    ...
    def create_participant(self, project_name: str, participant: ParticipantPostReq) -> ParticipantRes:
        ...
        if not FileValidators.is_valid_directory_name(participant.code):
            raise BadRequestException(
                f"Participant code {participant.code} isn’t allowed.")
    ...
    def exists(self, project_name: str, participant_code: str) -> bool:
        if not self.project_service.exists(project_name):
            raise BadRequestException(
                f"Project with name {project_name} doesn’t exist.")
